...
"""
    ...
    #                                                                        #
    ##########################################################################
    def ffmpeg(self, movie, files, quality="medium", rate=None, options=''):
        """
 Function: ffmpeg
 ...
 Example of Use:
    ...
    x.ffmpeg('mymovie.mpeg','my_png_%d.png', quality="high") # fast, low, medium, high, or bitrate ("512K", "1M")
...
"""
        cmd = 'ffmpeg -y '
        bitrate_re = re.compile("(\d+)([kKmMgG])?")
        if rate is not None:
        ...
        if isinstance(files, (list, tuple)):
            ...
            cmd += '-i ' + files
        if rate is not None:
            formatting = "-vf 'fps=%s,format=yuv420p'" % str(rate)
        else:
            formatting = "-vf 'format=yuv420p'"
        cmd += " %s" % formatting
        if quality == "fast":
            cmd += " -b:v 256k"
        elif quality == "low":
            ...
            if obj is None:
                ...
                if obj.group(2) is None:
                    ...
                    unit = obj.group(2).upper()
                cmd += " -b:v %s%s" % (value, unit)
        cmd += ' ' + options
        cmd += ' ' + movie
        print cmd
        o = os.popen(cmd).read()
